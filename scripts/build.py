...
# Build current-v1.js (a JS wrapper that sets `price_data` and calls a safe callback)
print('Building current-v1.js...')
js_output_path = project_dir / 'current-v1.js'
js_header = '''/* Generated by scripts/build.py - DO NOT EDIT */
/* NOTE: The requesting page (index.html) defines a callback named window.onPriceDataReady().
    When this file loads it will create a local `price_data` object and then
    call window.onPriceDataReady(price_data).
    This mechanism ensures index.html only parses the price data after it is available. */
'''

with open(js_output_path, 'w', encoding='utf-8') as f:
    f.write(js_header)
    # Write the JSON payload inside an IIFE closure so we don't leak globals
    start_closure = '''(function(){
  try {
    var price_data = '''
    f.write(start_closure)
    json.dump(current_json, f, ensure_ascii=False, indent=2)
    end_closure = ''';
    if (typeof window !== "undefined") {
      if (typeof window.onPriceDataReady === "function") {
        window.onPriceDataReady(price_data);
      } else {
        console.error("window.onPriceDataReady is not defined or not a function");
      }
    }
    else {
      console.error("window is not defined; cannot call onPriceDataReady");
    }
  } catch (e) {
    console.error("Error initializing price_data:", e);
  }
})();'''
    f.write(end_closure)

print('Created current-v1.js')
